...
| Typed config and defaults | `elixir/lib/symphony_elixir/config.ex`, `elixir/lib/symphony_elixir/config/schema.ex` | `go/internal/config`, `go/internal/runtimeconfig` | `internal/config` now owns typed workflow-backed runtime settings, worker config, raw-vs-local workspace-root handling, and runtime Codex policy resolution; `internal/runtimeconfig` remains the narrow process-local override store used by the CLI bootstrap. |
...
| Path canonicalization and containment | `elixir/lib/symphony_elixir/path_safety.ex` | `go/internal/pathsafety` | Canonicalize existing local path segments, preserve non-existent suffixes, and give workspace/Codex validation one shared notion of local path safety. |
| Workspace lifecycle and safety | `elixir/lib/symphony_elixir/workspace.ex` | `go/internal/workspace` | Derive deterministic local or remote workspace paths, enforce containment for local workspaces, preserve local work on reuse, and run hooks safely on both local and SSH-backed workers. |
| SSH transport | `elixir/lib/symphony_elixir/ssh.ex` | `go/internal/ssh` | Normalize SSH targets, honor optional SSH config, execute remote shell commands, and bootstrap remote stdio sessions. |
...
| Agent execution loop | `elixir/lib/symphony_elixir/agent_runner.ex` | `go/internal/agent` | Coordinate one issue attempt across workspace, prompt, Codex turns, issue refresh, preferred-worker retries, and SSH-host fallback. |
| Orchestrator and retries | `elixir/lib/symphony_elixir/orchestrator.ex` | `go/internal/orchestrator` | Own dispatch ordering, eligibility, worker-host selection, reconciliation, retries, snapshots, and aggregate runtime state. |
...
| Shared observability shaping | `elixir/lib/symphony_elixir_web/presenter.ex`, `elixir/lib/symphony_elixir_web/observability_pubsub.ex` | `go/internal/observability` | Build reusable snapshot payloads and shared presentation helpers for the API and later dashboard surfaces, including worker-host and workspace-path metadata. |
